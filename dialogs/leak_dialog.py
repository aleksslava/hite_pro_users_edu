...
async def leak_1_getter(dialog_manager: DialogManager, **kwargs):
    video_path = video_for_windows.get('leak_1')
    message = lexicon_ru.get('leak_1')
    return {
...
Format("{message}"),
    ...
    buttons,
    getter=leak_1_getter,
    state=Leak.stage_1
)

leak_dialog = Dialog(leak_1_window)
